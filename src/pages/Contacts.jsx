...
export default function Contacts() {
  ...
  const { data, isLoading, error } = useContacts({ page, limit: 10, search });
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Company & Title</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan="4" className="px-6 py-12 text-center text-sm text-gray-500">No contacts found.</td>
                </tr>
              ) : (
                  ...
                  <tr key={contact.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900 flex items-center">
                        {contact.company ? (
                          <Link to={`/companies/${contact.company.id}`} className="hover:text-indigo-600 flex items-center">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${contact.status === 'ACTIVE' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                        {contact.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link to={`/contacts/${contact.id}`} className="text-indigo-600 hover:text-indigo-900">View</Link>
                    </td>
                  </tr>
                ))
              )}
